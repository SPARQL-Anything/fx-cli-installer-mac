#!/bin/bash
install_dir="$HOME/.sparql-anything/bin"
#
#
#
#
...
#############################################################
#
# SPARQL Anything install location
if [[ -z "\$location" ]]; then
  location=~/.sparql-anything/bin/sparql-anything-SPARQL-ANYTHING-VERSION.jar
fi
#
...
#
# Test Java Version
javaversion=\$(javap -verbose java.lang.String | grep "major version" | cut -d " " -f5)
if [ "\$javaversion" -lt "55" ]; then
  echo "JDK version 11 or greater is required"
  exit 2
fi
#
...
# Remove old script
rm -f $install_dir/fx.sh
echo "$FXCLI" > "$install_dir/fx.sh" 
#
# Prepare download URL
download_url=$(curl -s https://api.github.com/repos/sparql-anything/sparql.anything/releases/latest\
|grep "browser_download_url.*g-0.*jar"\
| cut -d : -f 2,3 \
| tr -d \")
jar=$(echo $download_url | grep -o '[^/]*$')

if test -f "$install_dir/$jar"; then
...
#
# Set version in FX script
version=$(echo ${jar%.jar} | cut -d"-" -f3 )
echo "Installed version: "$version
sed -i.bu "s/SPARQL-ANYTHING-VERSION/$version/g" "$install_dir/fx.sh"
chmod +x "$install_dir/fx.sh"
#
# Update symlink
if command -v fx &> /dev/null
then
   echo "Updating link to latest version"
   rm /usr/local/bin/fx
fi
ln -s "$install_dir/fx.sh" /usr/local/bin/fx
echo "Command fx available at /usr/local/bin/fx. Type fx for help"
echo ""
echo "Issues at https://github.com/SPARQL-Anything/sparql.anything/issues"
echo ""
